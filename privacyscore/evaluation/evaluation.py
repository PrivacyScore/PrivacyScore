...
def evaluate_group(group: str, result: dict) -> GroupEvaluation:
    ...
    for check, data in CHECKS[group].items():
        ...
        descriptions.append((res['description'], data.get('title'), data.get('longdesc'), data.get('labels'), res['details_list'], res['classification'], res.get('severity', ""), res.get('finding', "")))
