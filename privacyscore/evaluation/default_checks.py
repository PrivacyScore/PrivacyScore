...
<p><strong>Conditions for passing:</strong> Referrer-Policy header is present. It is recommended to set Referrer-Policy to “no-referrer” or at least “same-origin”. As “same-origin” might not be recognized by all browsers, both “no-referrer” and “same-origin” (in this order) can be specified, to use “no-referrer” as fallback. These are the policies recommended by <a href="http://dataskydd.net">dataskydd.net</a> in their Webbkoll scan service.</p>
<p><strong>Reliability: unreliable.</strong> At the moment we only check for this header in the response that belongs to the first request for the final URL (after following potential redirects to other HTTP/HTTPS URLs).</p>
